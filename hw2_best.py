...
def _normalize(X, train=True, specified_column=None, X_mean=None, X_std=None):
    ...
    if train:
        # 這裡有改 我打算將X_Std去變成x_maximun以減少continuous項的影響力
        X_mean = np.mean(X[:, specified_column], 0).reshape(1, -1)
        X_std = np.std(X[:, specified_column], 0).reshape(1, -1)

    X[:, specified_column] = (
        X[:, specified_column] - X_mean) / (X_std + 1e-8)
...
# 在train 跟 test都新增3次項
column = [323, 515]
X_train_new = np.power(X_train[:, column], 3).reshape(-1, len(column))
X_test_new = np.power(X_test[:, column], 3).reshape(-1, len(column))
X_train = np.concatenate((X_train, X_train_new), axis=1)
X_test = np.concatenate((X_test, X_test_new), axis=1)

# 新增4次項
column = [502, 498]
X_train_new = np.power(X_train[:, column], 4).reshape(-1, len(column))
X_test_new = np.power(X_test[:, column], 4).reshape(-1, len(column))
X_train = np.concatenate((X_train, X_train_new), axis=1)
X_test = np.concatenate((X_test, X_test_new), axis=1)
...
X_test_new = (X_test[:, column1] *
              X_test[:, column2]).reshape(-1, len(column1))
X_train = np.concatenate((X_train, X_train_new), axis=1)
X_test = np.concatenate((X_test, X_test_new), axis=1)
# 超多重擬和

# 在train test刪除一些feature
# column = [0+what]
# X_train = np.delete(X_train, column, axis=1)
# X_test = np.delete(X_test, column, axis=1)
...
# column = [0+what]
# X_train_new = np.log(
#     np.abs(X_train[:, column])+1e-8).reshape(-1, len(column))
# X_test_new = np.log(
#     np.abs(X_test[:, column])+1e-8).reshape(-1, len(column))
# X_train = np.concatenate((X_train, X_train_new), axis=1)
# X_test = np.concatenate((X_test, X_test_new), axis=1)
...
for epoch in range(max_iter):
    ...
    train_loss.append(_cross_entropy_loss(
        y_train_pred, Y_train) / train_size)
